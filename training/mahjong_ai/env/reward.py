...
def _terminal_win_shaping(next_state: Any, player_id: int, cfg: dict) -> float:
    ...
    reward = 0.0
    terminal_win_bonus = float(cfg.get("terminal_win_bonus", 0.0))
    win_fan_scale = float(cfg.get("win_fan_scale", 0.0))
    self_draw = getattr(next_state, "win_type", None) == "self_draw"
    if self_draw:
    ...
    fan = 0
    if win_fan_scale or fan_bonus or point_bonus:
        score = score_hand(
            next_state,
            player_id,
        ...
        fan = int(score.get("fan", 0))
        reward += fan_bonus * float(fan)
        reward += point_bonus * float(score.get("points", 0.0))
    # Make bigger hands worth proportionally more than cheap wins, so the policy
    # is nudged toward value instead of mindless flat wins.
    reward += terminal_win_bonus * (1.0 + win_fan_scale * float(fan))
    return reward
